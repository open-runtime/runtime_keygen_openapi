...
class CreateLicenseRequestDataAttributes {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CreateLicenseRequestDataAttributes &&
    other.name == name &&
    other.key == key &&
    other.expiry == expiry &&
  ...
  @override
  String toString() => 'CreateLicenseRequestDataAttributes[name=$name, key=$key, expiry=$expiry, maxMachines=$maxMachines, maxProcesses=$maxProcesses, maxCores=$maxCores, maxUses=$maxUses, protected=$protected, suspended=$suspended, metadata=$metadata]';
  ...
  /// Returns a new [CreateLicenseRequestDataAttributes] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CreateLicenseRequestDataAttributes? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateLicenseRequestDataAttributes[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateLicenseRequestDataAttributes[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return CreateLicenseRequestDataAttributes(
  ...
  static List<CreateLicenseRequestDataAttributes> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CreateLicenseRequestDataAttributes>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CreateLicenseRequestDataAttributes.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CreateLicenseRequestDataAttributes> mapFromJson(dynamic json) {
    final map = <String, CreateLicenseRequestDataAttributes>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateLicenseRequestDataAttributes.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CreateLicenseRequestDataAttributes-objects as value to a dart map
  static Map<String, List<CreateLicenseRequestDataAttributes>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateLicenseRequestDataAttributes>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CreateLicenseRequestDataAttributes.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
